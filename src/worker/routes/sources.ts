...
export function registerSourceRoutes(app: Hono<AppBindings>): void {
  ...
  app.post("/api/sources", async (context) => {
    ...
    // Standalone sources must be manual and resolve to exactly one node —
    // validated against the real parser before anything is persisted.
    if (input.sourceKind === "standalone") {
      if (input.type !== "manual") throw new AppError(422, "单节点数据源必须是手动类型", "standalone_requires_manual");
      let parsed: Awaited<ReturnType<typeof parseSubscriptionContent>>;
      try { parsed = await parseSubscriptionContent(input.content ?? ""); } catch { throw new AppError(422, "节点链接无法解析", "invalid_standalone_uri"); }
      if (parsed.length !== 1) throw new AppError(422, "单节点数据源必须恰好包含一个节点", "standalone_requires_single_node");
    }
